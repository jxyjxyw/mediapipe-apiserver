...
class MediaPipeDetector:
    ...
    def get_landmarks(self, image_np: np.ndarray, require_annotation=True) -> Tuple[Optional[np.ndarray], List[List[Tuple[float, float]]]]:
        # convert image format
        image = mediapipe.Image(image_format=mediapipe.ImageFormat.SRGB, data=image_np)
            ...
            if zed.grab() == sl.ERROR_CODE.SUCCESS:
                ...
                if False:
                    ...
                    for i in range(len(uvsl)):
                        ...
                        ax.scatter(j3d[:, 0], j3d[:, 1], j3d[:, 2], c='b', s=100, alpha=0.5)
